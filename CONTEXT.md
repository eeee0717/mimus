...
| 18 | 译文字体收敛为 Noto Serif SC 2.001（宋体）+ STIX Two 两族：Han/CJK 及中文语境标点优先宋体，ASCII、拉丁、希腊、西里尔与数学/箭头等优先 STIX Two Text，Text 缺失的同家族符号再走 STIX Two Math，其余按宋体 → Text → Math；两族全缺才整段 `unsupported_font`。Regular/Bold 依旧样式保真，行高只取宋体 ascent/descent，同字号同基线不缩放。主槽沿用 `--font`/`--font-bold`；拉丁槽 canonical 为 `--font-latin`/`--font-latin-bold`、`MIMUS_FONT_LATIN(_BOLD)`、`font_latin(_bold)`，旧 fallback 名仅作弃用别名。各槽仍按 flag > env > config > SHA 缓存 > manifest 解析，经 `PassContext` 注入并按需子集化；字体身份不进翻译缓存键 | [ADR-0018](docs/adr/0018-output-font-assets.md) |
