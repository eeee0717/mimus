...
字体族刻意**不使用** Noto Sans SC（mimus 的译文输出字体）：输入字体与输出字体不同族，才能在输出 PDF 中区分"原文字形"与"译文字形"的来源。建议 DejaVu 或 Liberation 系。
...
**规模**：本矩阵共 **129 个失效模式**——解析与写回类 68 个（§3.1：PARSE 11 / STREAM 11 / FONT 10 / CMAP 9 / XOBJ 10 / GEOM 5 / WRITE 8 / DOC 4），版面与启发式类 61 个（§3.2–3.8：LAYOUT 8 / ORDER 4 / PARA 16 / FORM 14 / TABLE 3 / TYPE 12 / SCAN 4）。按优先级：**M0 27 个、M1 59 个、M3 43 个**（另有 ORDER-02、FORM-04、TYPE-01 三条跨两个优先级，按其首要标记归类）。
...
**DOC-03 · 加密 PDF 完全无处理（代码缺口）**
- 源码：全仓库非 vendored 代码中 `needs_pass` / `authenticate` / `is_encrypted` / `password` / `permissions` **零命中**。加密文档打开后流读取会抛异常，落到 `fix_null_xref` 的兜底 → **把整篇每个对象都改写成 `[]`** → "成功"输出一个空文档
- 触发：**空 user password 的加密 PDF**（防复制/防打印用途）——这是最常见的加密形式，完全合法可读
- V1：**高度相关**——lopdf 需显式处理 `/Encrypt`，否则 content stream 解出来是密文、走查产出乱码。**注意：CONTEXT.md 的现有决策未覆盖加密 PDF，这是一个范围缺口（见 §6）**
- 构造：(a) 空 user password 的 RC4 40-bit 加密；(b) AES-256（R6）空密码；(c) 设置了 user password 的
- 预期：(a)(b) 用空密码解密后正常翻译，输出保持加密属性或明确降级为未加密并告警；(c) 报 `PasswordRequired` 拒绝。**任何情况下不得产出空白或乱码文档**
- oracle：输出用 PDFium 打开提取文本，与未加密版本对比 + 退出码 · **M1** · 合法

**DOC-04 · 字符旋转角度判定失败后守卫落空**
- 源码：`il_creater_active.py:1292-1307`（`except` 分支里**没有 `return`**，角度算不出来的字符照样进入 IL，而角度算得出但不在 0°/90° 附近的字符被丢弃）；`il_creater.py:970-979` 同构
- 触发：字符矩阵含 NaN/Inf（由退化的 `Tm` 或除零产生）；任意角度旋转文本（45° 水印、旋转表头）在真实 PDF 中常见
- V1：相关
- 构造：一页含 0°、90°、45°、`0 0 0 0 100 700 Tm`（退化）四段文字
- 预期：0°/90° 正常翻译；45° 明确标记为非水平/垂直 → 不翻译但**原样保留**（不丢弃）；退化矩阵 → 标记不可定位并保留原文。断言输出中 45° 文本的 content stream 片段与输入相同
- oracle：光栅 diff（45° 区域像素差 == 0）+ 字符数断言 · **M3** · 合法（前三）/ 故意畸形（退化矩阵）
...
M1 覆盖矩阵中标为 M1 的 59 个 case。按域汇总（fixture 数含变体展开）：
...
M0 + M1 合计约 **132 份 fixture**（覆盖 86 个 case）——这远超早期那批 23 份，且每份都要过确定性、独立解析、独立渲染三重验收。诚实地说，这是一笔可观的前期投入。

三点缓解：

1. **M-1 只交付清单**，不交付 PDF。真正的生成工作按里程碑分批：M0 那约 45 份先行（它们直接解锁三个风险实验），M1 的 87 份随 pass 链逐步落地。
2. **基线 + 单变量变异的结构大幅摊薄成本**——多数 `mal-*` fixture 是对同一父本的一处字节改动，生成器写一次、变异描述写一行。
3. **优先级可再切分**：若 M0 的 45 份仍嫌重，可先做实验 2、3 所需的那批（走查与写回是 ADR-0006/0003 的核心假设），实验 1 所需的模型类 fixture 可用现实排版引擎快速产出。

矩阵中标为 M3 的 43 个 case 不在首批清单内，随质量攻坚阶段按需落地。
...
调研过程中暴露、但现有决策（`CONTEXT.md` / `docs/adr/`）未覆盖的事项。按需要决断的紧迫程度排列。

1. **加密 PDF 的 V1 策略是范围缺口（DOC-03）**。空 user password 的加密 PDF（防复制/防打印）完全合法、极其常见，而 CONTEXT.md 的 29 项决策没有一条涉及它。三个选项：(a) 支持空密码解密，输出保持或降级加密属性；(b) V1 一律拒绝并给分类错误；(c) 支持空密码、拒绝有密码。**建议 (c)**，因为拒绝一类常见合法输入会显著削弱"日常翻译 arXiv 论文"的验收场景——arXiv 本身不加密，但出版商 PDF 普遍加密。此决策会影响 lopdf 的使用方式，可能需要一篇 ADR。

2. **旧语料目录仍在磁盘上**。`~/Downloads/babeldoc-corpus/` 当前仍存在且含 28 项（23 份 PDF + 生成脚本 + MANIFEST + 联系表）。这与"已被删除并正式作废"的表述不符。本次工作全程未读取、未引用其任何几何参数或生成代码，但**该目录的实际删除需要你确认**——它留在原地就存在被后续误引用的风险。

3. **走查与 PDFium 不一致时的仲裁规则未定**。至少两处已知分歧：FONT-02（标准 14 内置度量 vs 文件 `/Widths` 的优先级）与 STREAM-02（多余操作数是跨操作符继承还是丢弃）。ADR-0006 说"PDFium text page 用作交叉校验"，但没说校验失败时以谁为准。建议在 M0 实验 2 的结论文档里确立一条明确规则（建议：以 PDF 规范为准，PDFium 与规范不符时以规范为准并记录偏差），否则每个分歧点都要重新争论一次。

4. **确定性生成的引擎侧机制尚未实测**（§2.6 已标注）。pdfTeX 的时间戳与 trailer ID 抑制、XeTeX 经 xdvipdfmx 的可重现性、Typst 的时间戳控制——都来自文档层面的认知。M-1 的工具链前置工作必须逐项验证"同一源文件生成两次 SHA-256 一致"，未通过的引擎不得用于现实排版 fixture。

5. **验收工具链缺四件**。本机现有 Ghostscript 10.07.1 与 TeX Live 2026（pdfTeX / XeTeX / LuaTeX）；缺 qpdf（结构解析）、poppler（文本坐标提取）、mupdf-tools（独立渲染 + 操作符 trace）、Typst。需补装并钉死版本记入 §2.5/§2.8。

6. **PP-DocLayoutV3 的 25 类是否含目录类未确认**（PARA-04 / 红利清单 D7）。若无目录类，目录页的条目切分需要 mimus 自行实现启发式；这会影响 M1 的工作量估计。

7. **任意角度旋转文本的政策未定（DOC-04）**。45° 水印、旋转表头在真实 PDF 中常见。矩阵中的建议是"不翻译但原样保留"，但这需要成为一条显式政策——它与"翻译政策表"是同一层级的决策，目前政策表只按版面类别划分，没有按文本方向划分。

8. **输入 fixture 的字体族选择待定**。§2.6 要求输入字体与输出字体（Noto Sans SC）不同族以便区分来源，拉丁侧用 DejaVu 或 Liberation 都合适；但 CJK 输入 fixture（如 CMAP-01 的 GBK 编码用例）需要一款 CJK 输入字体，可选范围小且体积大，可能不得不与输出字体同族——届时需要另一种区分来源的手段（如对象号或子集标签断言）。

9. **M0 的 45 份 fixture 是否需要再切分**（§4.4）。若希望更早跑通实验 2 与 3，可把实验 1 所需的模型类 fixture 后置——但这会推迟 ADR-0002 遗留的阅读顺序验证，而后者决定 midend 要写多少代码（红利清单 D1–D12）。这是个排期取舍，需要你定。
