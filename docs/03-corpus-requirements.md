...
> 状态：M-1 与 M0 已收口；74 份 M0 fixture 已按生成合同独立验收入库，M1/M3 fixture 随实现继续扩展
> 日期：制定于 2026-08-21，状态更新于 2026-08-23
...
**现实排版 fixture 的字体来源**：不额外 vendored 字体文件。Typst 用 `--ignore-system-fonts`，字体集合收敛为 Typst 二进制内嵌的那一份；TeX 引擎用 TeX Live 2026 自带的 Computer Modern 系列。两者都由已钉死的工具版本本身担保，且**字体完整嵌入产出的 PDF**，再 vendored 一份文件只是第二套 pin，不增加信息。上一段表格里"钉死字体文件 + SHA-256"的要求继续对**含文字的精确 fixture**成立——那批 fixture 的期望墨迹盒是手推的，字体度量必须逐字节可控。

**输入字体的选择**：含文字的精确 fixture 一律**嵌入钉死的字体文件**（记录 SHA-256），而非依赖 base-14 的隐式度量——否则"期望墨迹盒"取决于消费方的 base-14 度量表，不可判定。纯图与全空白精确 fixture 不得为满足门禁而伪造文字或字体。另一例外是专门测试 base-14 / 非嵌入字体处理的 fixture。
...
本节的确定性要求**不需要把加密 fixture 伪装成可重现**：V1 一律拒绝加密 PDF（ADR-0009），DOC-03 的 RC4 与 AES 两档均由 qpdf 12.4.0 一次性生成后入库，manifest 记录不经 shell 解释的完整 argv 与二进制 SHA-256，不参与往返。
...
- 收口注记（2026-08-23）：空白/扫描口径已由 ADR-0012 定为三元分类（空白=0 文字对象且无图像；扫描=有图像且 0 可见文字对象，`Tr 3` 不算可见）；空白页与扫描页豁免严格 walk、整页透传。预期不变
...
- 收口注记（2026-08-23）：结构信号方案已由 ADR-0012 采纳（不用 SSIM/光栅）。(a)(b)(d) 拒绝维持；统计字段落地为 error 事件 `scanned_pages`/`total_pages` + 单条汇总 diagnostic。(c) 预期修订：按原生分类（不误判为扫描），但严格 walk 尚不支持 `Do`，暂以 `unsupported_pdf` 拒绝，#18 解除
...
#### PARA-17 · 单个模型区域内的并列作者列
- 触发：版面模型用一个 `text` region 覆盖两列作者信息；前两行的栏间距足以形成独立
  line，末行栏间距略小于通用断行阈值
- V1：**相关**——若直接按 baseline 排序再切自然段，会把同一行的左右列交错归并，
  甚至让一位作者吞入两列机构与邮箱
- 构造：一个 region 内两列、每列三行；右列 x 锚点固定，左列因居中而逐行变动，末行
  两列字符间距小于 `1.8 × font_size`
- 预期：先由至少两行一致的列间隔证明双列结构，再沿共同 gutter 分割末行；输出恰为
  两个按视觉列归属的段落，无法证明共同 gutter 时保持既有自然段路径
- oracle：手推字体几何 + recorded layout + ParagraphFind IL · **M3** · 合法
...
- 收口注记（2026-08-23）：单页判据已由 ADR-0012 定为纯结构信号（有图像且 0 可见文字对象）。(a) 拒绝、(c)(d) 正常翻译维持。**(b)(e) 预期修订**：可见文字层与普通图文页结构不可区分，统一按原生分类（不误判为扫描），但暂以 `unsupported_pdf` 拒绝（严格 walk 尚不支持 `Do`），#18 解除；fixture 亦合并为一档
...
- 收口注记（2026-08-23）：聚合规则已由 ADR-0012 定为「扫描页 ≥ 80% × 内容页（内容页=总页−空白页，含等于）即拒绝」，全量计数无提前退出。(a) 单页 100% 必拒；(b)(c) 75% < 80% → 都继续且结论相同；(d) 定为继续处理，扫描页豁免严格 walk 整页透传 + 汇总 diagnostic 标记；20 页档 fixture 不单独入库（低比例继续已被 4 页混合与 PARSE-09 变体覆盖），逐页断言改为断言 diagnostic 的 `scanned_page_indices`
...
- 预期：(a) 拒绝；(b)(c) 正常翻译
- oracle：退出码 · **M1** · 合法
- 收口注记（2026-08-23）：`Tr 3` 已进入 ADR-0012 的可见性口径（隐形文字不计入可见文字数），非独立正则快筛。(a) 拒绝、(b) 正常翻译维持。**(c) 预期修订**：分类为原生（可见文字 > 0，不误判为扫描），但严格 walk 尚不重放 `Tr`，暂以 `unsupported_pdf` 拒绝，Tr 重放支持后解除
...
| D7 | 目录点线切段 | YOLO 无目录类 | **部分兑现**：`content` 可识别目录整块，不提供条目粒度 | PARA-04 | 仍需自行切分目录条目与页码 |
...
### 4.2 M0 批次（已完成，服务于三个风险探测实验）

> **执行结果（2026-08-23）**：三个实验全部通过，共 74 份 M0 fixture 入库。下列清单保留 M-1 阶段的 case-to-fixture 规划与实际执行路径；其中“约 45 份”是当时的粗估，不是最终库存。实际库存以 `corpus/fixtures/` 及其 manifest 为准。

#### 原排期与执行方式：不等齐规划中的约 45 份（决策 #33）

执行时没有等待规划内全部 fixture 齐备。**每个实验的最小 fixture 集独立验收通过后，该实验即刻启动。**
...
规划中余下约 35 份随对应实验推进补齐。**M-1 始终整体收口**——本节切分只影响 M0 内部的启动顺序，没有改变 M-1 的四条收口断言。
...
**M0 规划估算约 45 份**（含变体展开）；执行中为覆盖验证分支扩展到 **74 份**，且均已通过入库门禁。
...
M-1 阶段规划时，M0 + M1 合计约 **138 份 fixture**（覆盖 87 个 case）。该数字是容量估算，不是最终库存；M0 实际已入库 74 份，M1 将随 pass 实现扩展。每份仍须通过确定性、独立解析与独立渲染验收。

三点缓解：

1. **M-1 只交付清单**，不交付 PDF。真正的生成工作按里程碑分批：M0 fixture 已先行入库并解锁三个风险实验；M1 的规划项随 pass 链逐步落地。
2. **基线 + 单变量变异的结构大幅摊薄成本**——多数 `mal-*` fixture 是对同一父本的一处字节改动，生成器写一次、变异描述写一行。
3. **M0 内部按实验切分执行**（§4.2）：最小首批 10 份验收通过即启动对应实验，没有等待规划清单全部完成。实验 1 因为不依赖自建 writer，最先启动。
...
4. ~~**PP-DocLayoutV3 的 25 类是否含目录类未确认**~~ → **M0 实验 1 已收口**。`content` 在 `unit-para-04-toc` 上以 0.955 识别整个目录块，但不提供条目/页码粒度；D7 因而是“类别层面兑现、切分层面未兑现”，生产实现仍需自行切分。见 [M0 实验 1](04-m0-experiment-1.md) §4 D7。

5. **CJK 输入 fixture 的字体选型**。可选范围小且体积大，可能不得不与输出字体（Noto Sans SC）同族。溯源手段已不依赖字体族差异，所以这纯粹是选型与体积取舍，不再是设计问题；在 M1 Issue #19 中执行。

### 6.3 由实验给出结论，不是决策

6. ~~**走查与 PDFium 不一致时的仲裁规则未定**~~ → **M0 实验 2 已收口**。PDF 规范 + hand-written manifest + 独立结构/字体/几何推导是事实层；独立 parser/renderer trace 验证落地；PDFium 只作交叉证据。规范允许多种恢复时选择有界、可报告、不扩散的路径，无法可靠恢复则降级。FONT-02、CMAP-04、STREAM-02、Type3 与 inline image 均已按此规则裁定，见 [M0 实验 2](04-m0-experiment-2.md) §7。

M-1 与 M0 至此没有仍需拍板或由实验回答的遗留项。M1/M3 fixture 继续按本合同随实现逐批落地；本节唯一尚未执行的工作是 #19 中的 CJK 输入字体选型。
