...
1. **加密 PDF 的 V1 策略是范围缺口（DOC-03）**。空 user password 的加密 PDF（防复制/防打印）完全合法、极其常见，而 CONTEXT.md 的 30 项决策没有一条涉及它。三个选项：(a) 支持空密码解密，输出保持或降级加密属性；(b) V1 一律拒绝并给分类错误；(c) 支持空密码、拒绝有密码。**建议 (c)**，因为拒绝一类常见合法输入会显著削弱"日常翻译 arXiv 论文"的验收场景——arXiv 本身不加密，但出版商 PDF 普遍加密。此决策会影响 lopdf 的使用方式，可能需要一篇 ADR。
