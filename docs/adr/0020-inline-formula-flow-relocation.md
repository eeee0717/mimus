...
3. #95 的固定公式槽计划已经失败；
4. 每个公式 text-show span 只包含该 inline-formula 单元，不与可翻文本共享操作数；
...
因此公式的源字体对象号、子集标签、操作数字节、Unicode/code 和单元内部相对几何不变；
只有已翻译段内 inline formula 的绝对 baseline/box 可以改变。输出 round-trip validator
...
- 字节/身份 hash：操作数字节、Unicode/code、字体对象号与子集标签，StylesAndFormulas
  至 Write 必须不变；
...
- 剩余 `(4,9)`（69 Han）与 `(8,79)`（138 Han）的 text/formula 共享同一 operand，因不
  满足 §1 的独占 span 前提而以 `typeset_protocol` 整段 fail closed；两段合计 207 Han，
  不再记为 `typeset_overflow`。
- 写回会修改更多 operand span，因此 retained-character 账目必须把重定位公式从“保留字符”
  转入“带新 baseline 的预期字符”。
- 任何共享 operand、跨行公式或奇异 content transform 继续 fail closed；后续真实语料若证明
  这些形状重要，须另立 ADR 扩大边界。
