...
- **严格 walk 原样不动**（白名单、fail-closed 语义均不变），执行时机挪到 ScanDetect 之后、且只跑「将被翻译的页」（§2 豁免规则）。
  &gt; **2026-08-24 修订（ADR-0013）**：「原样不动」这半句已被 [ADR-0013](0013-bounded-walk-and-graded-degradation.md) 取代——walk 从白名单 fail-closed 变为有界宽容走查。本节其余条款（宽容预扫永不报错、证据不完整保守归内容页、执行时机在 ScanDetect 之后、只跑将被翻译的页、统计放 `ExtractedPage`、公开 IL 保持 v1）全部不变。§1 的可见性口径尤其不变：`Tr 3`/`Tr 7` 不可见与非直立是两条正交维度，非直立字符继续计入可见文字统计。分类优先于保真检查——这是「纯图页必须报 `scanned_pdf` 而非 `unsupported_pdf`」的验收标准强制的。实现建议并入 Layout 开头、不新增公开 stage 值；若实现时改为独立 stage，新 stage 值属 ADR-0011 §6 的 v2 兼容扩展。
...
**扫描 12 份**（去重合并 SCAN-01/02/03 与 DOC-02 的构造，ID 实现时定稿；连同上面的加密 2 份，共新增 14 份）：
