...
发射策略只报告可行动证据：C 强/弱冲突、未解析、walk-only、engine-only、其余残差或
baseline 残差任一非零时才发 `engine_character_alignment`。仅有提取视图等价类或解释边
计数的页面继续完成同一套分类与 per-character 对齐，但不发公开诊断，也不消耗诊断预算。
该抑制不改变匹配、保留或 `tight_box` 决策；相关计数仍由对应 fixture 固定。
