...
| `--font`, `--font-bold`, `--font-latin`, `--font-latin-bold` | pinned Noto Serif SC + STIX Two Text/Math 20-paper sweep and font tests | pinned files on selected runs | all layers | PASS for the pinned set; deprecated fallback names remain aliases; no broad user-font matrix. |
