...
fn doc_title_passthrough_keeps_tight_source_identity() {
    ...
    let server = GateResponsesServer::start([]);
    let output = run_openai(
        "unit-type-01-single-line-tight",
        &server,
        RunOptions {
            output: &output_path,
            debug: Some(&debug),
            cache: None,
            model: "m3-tight-title-model",
            target_language: "zh-CN",
            glossary: None,
            auto_terms: false,
            strict: false,
...
fn shared_tj_and_tj_array_formula_operands_split_without_replaying_source_text() {
    ...
    let directory = tempfile::tempdir().unwrap();
    let debug = directory.path().join("debug");
    let output_path = directory.path().join("shared-formula-operand.pdf");
    let server = GateResponsesServer::start([ScriptedReply::Output(TRANSLATION)]);
    let output = run_openai(
        "unit-type-13-shared-formula-operand",
        &server,
        RunOptions {
            output: &output_path,
            debug: Some(&debug),
            cache: None,
            model: "m3-shared-formula-operand-model",
            target_language: "zh-CN",
            glossary: None,
            auto_terms: false,
            strict: false,
    ...
    assert!(events.iter().all(|event| {
        event["id"] != "degradation_summary"
            || event["preserved_paragraphs"]
                ...
                .unwrap()
                .iter()
                .all(|paragraph| paragraph["reason"] != "typeset_protocol")
    }));
    for extractor in ["poppler", "mupdf"] {
        let extracted = extract_pdf_text(&output_path, extractor);
        let han = extracted
    ...
    let rewritten = std::str::from_utf8(streams.last().unwrap()).unwrap();
    for replay in [
        "/F1 12 Tf\n1 0 0 1 82.356 140 Tm <49> Tj",
        "/F1 12 Tf\n1 0 0 1 96.252 126 Tm <55> Tj",
        "/F1 12 Tf\n1 0 0 1 72 112 Tm <4D> Tj",
        "/F1 12 Tf\n1 0 0 1 82.356 112 Tm <49> Tj",
        "/F1 12 Tf\n1 0 0 1 85.896 112 Tm <4D> Tj",
        "/F1 12 Tf\n1 0 0 1 96.252 112 Tm <55> Tj",
    ] {
        assert!(
            rewritten.contains(replay),
            "source formula byte/font replay missing from {rewritten}"
        );
    }
    for state_tail in [
        "1 0 0 1 72 140 Tm\n[-3388] TJ\n()",
        ...
        assert!(
            rewritten.contains(state_tail),
            "source show advance was not restored in {rewritten}"
        );
    }
    assert_valid_pdf(&output_path, "unit-type-13-shared-formula-operand");
    server.assert_clean();
}

#[test]
fn short_identity_text_in_shared_formula_operands_uses_the_same_safe_split_path() {
    let directory = tempfile::tempdir().unwrap();
    let output_path = directory.path().join("shared-formula-operand-identity.pdf");
    let server = GateResponsesServer::start([ScriptedReply::Echo, ScriptedReply::Echo]);
    let output = run_openai(
        "unit-type-13-shared-formula-operand",
        &server,
...
fn every_legal_fixture_uses_the_loopback_responses_gate() {
    ...
    assert_eq!(ids.len(), 158, "Corpus fixture inventory changed");
    assert_eq!(unique_cases.len(), 93, "Corpus case inventory changed");
    assert_eq!(legal.len(), 116, "legal fixture inventory changed");
