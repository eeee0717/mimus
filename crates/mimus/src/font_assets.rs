...
pub(crate) fn resolve_fonts(
    ...
    resolve_with_manifest(regular, bold, cache_dir, mirror, &production_manifest())
}
...
fn resolve_one(
    ...
    if actual_sha256 != descriptor.sha256 {
        ...
        .with_hint("check the configured asset mirror or provide --font and --font-bold"));
    }
...
fn load_custom_font(path: &Path, source: &'static str) -> Result<OutputFont> {
    let bytes = std::fs::read(path).map_err(|_| {
        ...
        .with_hint("provide readable Regular and Bold files with --font and --font-bold")
    })?;
...
fn parse_font(bytes: Vec<u8>, source: String) -> Result<OutputFont> {
    let face = ttf_parser::Face::parse(&bytes, 0).map_err(|_| {
        ...
        .with_hint("provide TTF or OpenType Regular and Bold files with --font and --font-bold")
    })?;
...
fn missing_fonts_error() -> MimusError {
    MimusError::asset(
        AssetReason::OutputFontUnavailable,
        "Regular and Bold output fonts could not be resolved",
    )
    .with_hint("provide --font and --font-bold or configure MIMUS_ASSET_MIRROR")
}
...
mod tests {
    ...
    fn loopback_download_is_sha_checked_published_and_then_read_from_cache() {
        ...
        let downloaded =
            resolve_with_manifest(None, None, cache.path(), Some(&base_url), &manifest).unwrap();
        server.join().unwrap();
        assert!(
            downloaded
                .regular
